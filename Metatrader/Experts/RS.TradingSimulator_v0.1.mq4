...
string GetCurrentPrices() {
    ...
    int selectedBar = bar - bars_count;
    if(selectedBar < 0)
    {
        Print("Please position the chart in a past time");
        selectedBar = 0;
    }
    Print("Prices at bar " + (selectedBar));
    Print("Close " + Close[selectedBar]);

    CJAVal jv;
    jv["Low"] = Low[selectedBar];
    jv["High"] = High[selectedBar];
    jv["Open"] = Open[selectedBar];
    jv["Close"] = Close[selectedBar];
